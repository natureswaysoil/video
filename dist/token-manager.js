...
exports.checkInstagramTokenExpiry = checkInstagramTokenExpiry;
exports.ensureValidInstagramToken = ensureValidInstagramToken;
exports.getTwitterTokenInfo = getTwitterTokenInfo;
exports.getYouTubeTokenInfo = getYouTubeTokenInfo;
 ...
 */
async function refreshInstagramToken(currentToken, appId, appSecret) {
    try {
        ...
        throw new errors_1.AppError(`Instagram token refresh failed: ${error?.response?.data?.error?.message || error.message}`, errors_1.ErrorCode.AUTH_ERROR, 401, true, { hasAppCredentials: !!(appId && appSecret) }, error);
    }
}
/**
 ...
 */
async function checkInstagramTokenExpiry(accessToken, appId, appSecret) {
    try {
 ...
 */
async function ensureValidInstagramToken(currentToken, appId, appSecret) {
    try {
 ...
 */
function getYouTubeTokenInfo() {
    return {
        requiresRefresh: false,
        note: 'YouTube tokens are automatically refreshed by googleapis library',
    };
}
function resolveSecretParentPath() {
    const secretLocator = process.env.INSTAGRAM_ACCESS_TOKEN_SECRET_NAME || process.env.GCP_SECRET_INSTAGRAM_ACCESS_TOKEN;
    if (!secretLocator) {
        return null;
    }
...
async function persistInstagramToken(newToken) {
