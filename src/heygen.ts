...
function fallbackBroll(input: BlogVideoInput): string[] {
  ...
  if (/dog|urine|pet|odor|yellow spot/.test(text)) return ['dog on green lawn', 'yellow lawn spots grass', 'homeowner spraying lawn', 'healthy green turf close up']
  if (/pasture|hay|field|farm|acre|horse|cattle/.test(text)) return ['green pasture field', 'tractor spraying pasture', 'healthy forage close up', 'farm soil grass roots']
  if (/orchid|house plant|indoor/.test(text)) return ['orchid plant close up', 'potting soil indoor plants', 'houseplant watering', 'healthy roots potting mix']
  if (/biochar|compost|worm|casting|soil|humic|fulvic|kelp/.test(text)) return ['hands holding rich soil', 'garden soil close up', 'raised bed garden', 'healthy plants']
...
function backgroundForScene(scene: any, fallbackImage?: string) {
  if (isHttpUrl(scene?.brollUrl)) return { type: 'video', url: String(scene.brollUrl), play_style: 'loop' }
  if (isHttpUrl(scene?.imageUrl)) return { type: 'image', url: String(scene.imageUrl) }
  if (isHttpUrl(fallbackImage)) return { type: 'image', url: String(fallbackImage) }
  return undefined
}
...
export class HeyGenClient {
  ...
  async createVideoJob(payload: any): Promise<string> {
    ...
    const videoInputs = scenes.map((scene: any, index: number) => {
...
export async function generateHeyGenVideo(input: BlogVideoInput): Promise<BlogVideoResult> { return generateOpenAIBlog(input) }
export async function createHeyGenVideo(input: BlogVideoInput): Promise<BlogVideoResult> { return generateOpenAIBlog(input) }
export async function createVideo(input: BlogVideoInput): Promise<BlogVideoResult> { return generateOpenAIBlog(input) }
export async function generateBlogVideo(input: BlogVideoInput): Promise<BlogVideoResult> { return generateOpenAIBlog(input) }
export async function generateBlogPackage(input: BlogVideoInput): Promise<BlogVideoResult> { return generateOpenAIBlog(input) }
...
export default { createClientWithSecrets, generateHeyGenVideo, createHeyGenVideo, createVideo, generateBlogVideo, generateBlogPackage, saveBlogMarkdown }
