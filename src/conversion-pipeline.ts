...
type PriorityProductPlan = {
  key: string
  match: RegExp
  landingPath: string
  priority: number
  recommendedPostsPerWeek: number
  variations: Array<{ angle: string; hook: string; visual: string; cta: string }>
}

export const PRIORITY_PRODUCT_PLANS: PriorityProductPlan[] = [
  {
    key: 'dog-urine',
...
function getPriorityPlan(product: ProductLike): PriorityProductPlan | undefined {
  const text = [
  ...
  return PRIORITY_PRODUCT_PLANS.find((plan) => plan.match.test(text))
}
...
function buildWebsiteUrl(product: ProductLike): string {
  ...
  if (plan) return `${base}${plan.landingPath}`
...
function categoryFromText(text: string): string {
  ...
  if (/(fruit tree|apple|peach|pear|citrus|banana|bloom|fruit set)/.test(lower)) return 'fruit-tree'
  if (/(hay|pasture|forage|horse|livestock)/.test(lower)) return 'hay-pasture'
  if (/(lawn fertilizer|lawn care|grass|turf|seaweed.*humic|humic.*lawn)/.test(lower)) return 'lawn-fertilizer'
